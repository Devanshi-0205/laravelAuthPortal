...
<body>
    ...
    <div class="container">
        ...
        <form action="{{ route('register.admin') }}" method="post" novalidate id="registrationAdminForm">
            @csrf
            <div class="form-row">
        ...
        @if (!Auth::user())
            <div>
                <span style="position:relative; top:10px;">Already an user? <a href="{{ route('login.admin') }}">
                        Login
                        from here </a></span>
            </div>
        @endif
    ...
    <script>
        $(document).ready(function() {
            $("#registrationAdminForm").validate({
                rules: {
                    fname: {
                        required: true,
                        minlength: 5,
